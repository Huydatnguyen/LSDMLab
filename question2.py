...
scheduling_class_RDD = job_events_RDD_combined.map(lambda x: x[Job_events_table.SCHEDULING_CLASS])
...
for elem in scheduling_class_list:
    ...
    if elem != '':
       ...
       print("Percentage of jobs correspond with scheduling class =", elem ,"is", round(count/sum_of_elements * 100 , 2) , "%  " , count,"of",sum_of_elements)
